...
export function SettingsForm({
  ...
  const captureShortcut = () => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (
        displayKey &&
        !['Control', 'Shift', 'Alt', 'Meta'].includes(displayKey)
      ) {
  ...
  const positionOptions: {
    ...
    {
      value: 'center',
      label: t('settings.posCenter'),
      desc: t('settings.posCenterDesc'),
      icon: Icon.center,
    },
    {
      value: 'cursor',
      label: t('settings.posCursor'),
      desc: t('settings.posCursorDesc'),
      icon: Icon.cursor,
    },
  ];
  ...
  return (
    <div className="modal modal-settings" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="modal-head">
        <h2>{title ?? t('settings.title')}</h2>
        <button className="icon-btn" onClick={onClose} title={t('clip.close')}>
          ✕
        </button>
      </div>
      ...
      <div className="modal-body modal-scroll">
        {/* ── 快捷键 ── */}
        <Section icon={Icon.keyboard} title={t('settings.section.shortcut')}>
          <label className="s-field">
            <span className="s-label">{t('settings.globalShortcut')}</span>
            <div
              className={`shortcut-capture ${capturing ? 'capturing' : ''}`}
              onClick={captureShortcut}
            >
        ...
        {/* ── 面板位置 ── */}
        <Section icon={Icon.cursor} title={t('settings.section.position')}>
        ...
        {/* ── 存储与清理 ── */}
        <Section icon={Icon.storage} title={t('settings.section.storage')}>
          <label className="s-field">
            <div className="s-info">
              <span className="s-label">{t('settings.maxItems')}</span>
              <span className="s-desc">{t('settings.maxItemsDesc')}</span>
            </div>
            <div className="num-input">
              ...
              />
              <span className="num-unit">{t('settings.itemsUnit')}</span>
            </div>
          </label>
          <label className="s-field">
            <div className="s-info">
              <span className="s-label">{t('settings.cleanupBatch')}</span>
              <span className="s-desc">{t('settings.cleanupBatchDesc')}</span>
            </div>
            <div className="num-input">
              <input
                type="text"
                ...
                onBlur={() => {
                  const val =
                    draft.cleanupBatch >= 10 ? draft.cleanupBatch : 10;
        ...
        <Section icon={Icon.paste} title={t('settings.section.behavior')}>
          <div className="s-field-row">
            <div className="s-info">
              <span className="s-label">
                {t('settings.pasteOnDoubleClick')}
              </span>
              <span className="s-desc">
                {t('settings.pasteOnDoubleClickDesc')}
              </span>
            </div>
            <Toggle
              checked={draft.pasteOnDoubleClick}
              onChange={(v) => save({ pasteOnDoubleClick: v })}
            />
          </div>

          <div className="s-field-row">
            <div className="s-info">
              <span className="s-label">{t('settings.launchAtLogin')}</span>
              <span className="s-desc">{t('settings.launchAtLoginDesc')}</span>
            </div>
            <Toggle
        ...
        <Section icon={Icon.palette} title={t('settings.section.appearance')}>
          <label className="s-field">
            <span className="s-label">{t('settings.themeMode')}</span>
            <select
              value={draft.theme}
        ...
        <Section icon={Icon.refresh} title={t('settings.section.about')}>
          <div className="s-field-row">
            <div className="s-info">
              <span className="s-label">{t('settings.checkUpdate')}</span>
              <span className="s-desc">
                {appVersion ? `Shelf v${appVersion}` : 'Shelf v—'}
              </span>
            </div>
            <button
              className="s-btn"
              onClick={() => void getClip().checkUpdate()}
            >
              {t('settings.check')}
            </button>
          </div>
