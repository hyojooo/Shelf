...
function applyShortcut(accel: string): boolean {
  ...
  if (ok) {
    ...
    registerShortcut(currentAccelerator, () =>
      toggle(getSettings().panelPosition),
    );
  }
  return ok;
}
...
async function bootstrap(): Promise<void> {
  ...
  createTray(
    {
      show: () => showPanel(getSettings().panelPosition),
  ...
  ipcMain.handle(
    IPC.FAVORITE,
    async (_e, payload: { id: string; favorite: boolean }) => {
  ...
  ipcMain.handle(
    IPC.UPDATE_SETTINGS,
    async (_e, partial: Partial<Settings>) => {
      ...
      if (
        typeof partial.maxItems === 'number' ||
        typeof partial.cleanupBatch === 'number'
      ) {
        store.setLimits(next.maxItems, next.cleanupBatch);
      }
      if (typeof partial.language === 'string') {
        setTrayLanguage(partial.language);
      }
