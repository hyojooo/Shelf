...
// 安全的渲染进程 API 桥接（contextIsolation 开启）
// 渲染进程通过 window.clip 调用；所有通信经 ipcRenderer 完成，不暴露 Node 能力。
const api = {
  ...
  deleteClip: (id: string): Promise<boolean> => ipcRenderer.invoke(IPC.DELETE, { id }),
  clearAll: (): Promise<boolean> => ipcRenderer.invoke(IPC.CLEAR),
  copy: (id: string): Promise<boolean> => ipcRenderer.invoke(IPC.COPY, { id }),
  paste: (id: string): Promise<boolean> => ipcRenderer.invoke(IPC.PASTE, { id }),
  ...
  // 订阅主进程事件：历史更新 / 设置更新 / 更新状态 / 打开设置 / 可见性变化 / 快捷键冲突
